...
Скопируйте `.env.example` в `.env` и переопределите только нужные значения. Бот сначала читает `.env` из корня проекта, затем `htxbot/.env` только для ещё не заданных переменных; profile overrides с префиксами `LONG_`/`SHORT_` задаются в выбранном общем файле.
...
HTX_API_SECRET=
COINS=aave,ada,...
# Опциональный второй ключ для монет, разрешённых на другом HTX API key:
HTX_API_KEY_2=
HTX_API_SECRET_2=
COINS_2=1inch,aixbt,...
BOT_PROFILES=long,short
...
- Не коммитьте `.env` или `htxbot/.env`.
