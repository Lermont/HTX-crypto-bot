...
Проект рассчитан на трейдеров и разработчиков, которым нужен crypto trading bot с простым стартом, гибкой настройкой через `.env` и уже проработанной EMA Pullback strategy из коробки. По умолчанию включён `DRY_RUN=true`, поэтому первый запуск проверяет конфигурацию, загрузку рынков, генерацию сигналов, логи и планируемые ордера без отправки реальных заявок.
...
- **Dry-run по умолчанию**: `DRY_RUN=true` защищает первый запуск от реальных ордеров.
...
- Доступ к USDT-M futures при `DRY_RUN=false`.

Установка и dry-run запуск:
...
Первый dry-run запуск должен показать загрузку рынков, выбор BTC benchmark, обновление сигналов, решения entry gate и планируемое поведение ордеров без реальных сделок.
