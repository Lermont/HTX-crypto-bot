...
Скопируйте `.env.example` в `.env` и переопределите только нужные значения. Бот читает только `.env` из корня проекта; profile overrides с префиксами `LONG_`/`SHORT_` задаются в этом же файле.
